 ...
 * Project: CPSC 597
 * Description: This file will register and enroll the Org2 App User which will be the indentity
	...
	try {
		...
		console.log(`Setting new wallet path to ${walletPath}`);
 		...
 		// Of course, let's put a check in here to make sure the user doesn't already
                // exist.
		const userIdentity = await wallet.get('org2-app-user');
		if (userIdentity) {
			console.log('An identiy for the user "org2-app-user" already exists ' +
				'in the wallet');
			return;
		}
		...
		const adminIdentity = await wallet.get('org2-app-admin');
		if (!adminIdentity) {
			console.log('An identity for the admin user "org2-app-admin" does not ' +
				'yet exist in wallet...');
		...
		// admin information.
		console.log('Using org2-app-admin identity for authentication...');
		const provider = wallet.getProviderRegistry().getProvider(adminIdentity.type);
		const adminUser = await provider.getUserContext(adminIdentity, 'org2-app-admin');

		// Now we are free to register the user, enroll the user, and import that new 
		// identity into the wallet.
		console.log('Registering org2-app-user...');
		const secret = await ca.register({
			enrollmentID: 'org2-app-user',
			role: 'client'
		}, adminUser);
		console.log('Enrolling org2-app-user...');
		...
		await wallet.put('org2-app-user', x509Identity);
		console.log('Successfully registered and enrolled admin user "org2-app-user" ' +
			'and imported it to wallet');

	// Again, catch any falling errors.
	} catch (error) {
		console.error(`Error: Failed to register user "org2-app-user": ${error}`);
		process.exit(1);
	}
}

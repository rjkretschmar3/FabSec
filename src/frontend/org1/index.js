...
async function initEnvironAndRunFrontendServer() {
	...
	// First, we need to grab a copy of the org1-app-user's credentials. The creation of these 
	...
	const credPath = path.resolve(__dirname, '..', '..', 'backend', 'org1', 'wallet', 'org1-app-user.id');
	fs.copyFile(credPath, './wallet/org1-app-user.id', (err) => {
		if (err) {
			console.log('An identity for the user "org1-app-user" does not seem to exist in the ' +
				'backend wallet...');
			console.log('Run the registerUser.js in the org1\'s backend directory before retrying.');
	...
	// comments.
	console.log('Connection through Gateway as "org1-app-user" with Discovery...');
	...
	// Fire up the server!
	var fabsecDemoServer = fabsecDemoApp.listen(8081, "127.0.0.1", () => {
		var host = fabsecDemoServer.address().address;
		var port = fabsecDemoServer.address().port;

		console.log(`FabSec Demo Server - Org1 listening on http://${host}:${port}`);
	});
}

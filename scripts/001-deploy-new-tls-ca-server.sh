...
# This script will initialize and deploy an Organization's TLS CA Server. It assumes a few things:
...
#
#	The directory infrastructure is already in place. For example, a peer organization of ID 1 would have the directory structure of organizations/peerOrganizations/org1.fabsec.com 
#
...
# First move to the appropriate directory: organizations/ordererOrganizations for an Orderer and organizations/peerOrganizations for a Peer.
if [[ "$1" == "orderer" ]]; then
...
#	1) It will register a bootstrap identity, which will be in the form of <ADMIN_USER>:<ADMIN_PW>. In the case of the above example the bootstrap identity will be org1-tls-admin:org1-tls-adminpw.

...
async function enrollAdmin() {
	try {
		...
		// exist. 
		const identity = await wallet.get('org1-app-admin');
		if (identity) {
			console.log('An identity for the admin user "org1-app-admin" ' + 
		...
		const enrollment = await ca.enroll({ 
			enrollmentID: 'org1-fab-admin', 
			enrollmentSecret: 'org1-fab-admin-pw'
		});
		...
		await wallet.put('org1-app-admin', x509Identity);
		console.log('Successfully enrolled admin user "org1-app-admin" and imported their' +
			' identity into the wallet');

	// Catch any falling errors. For now, we can leave this pretty general to just guide debugging.
	} catch (error) {
		console.error(`Error: Failed to enroll admin user "org1-app-admin" => ${error}`);
		process.exit(1);
	}
}
